...
WARNING!!: There will be a few intentional errors here for you to fix. If a line of code isn't working, think about how you can fix it to make it work. 
...
both <- both %>%
  ...
  mutate(`Subject ID` = factor(as.numeric(`Subject ID`)))
```
...
geom_jitter is a great way to make a scatterplot with a categorical variable on the x-axis. It spreads the points out so that they do not all line up exactly for the same values of x.
...
We like to add informative labels with the `labs` layer to improve on our variable names. 
...
Now, let's fit a model with multiple covariates. Let's consider Treatment and Sample Type. 
