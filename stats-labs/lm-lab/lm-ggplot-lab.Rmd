...
Now, let's fit a model with multiple covariates. Let's consider Treatment and Sample Type. Wrtie the proper model below, using `+` to connect covariates
...
1. Two covariates that you think could potentially be associated to the mean ddpcr in some scale. 
2. Whether if you are interested in exploring these associations in the linear scale or employ a generalized linear model. Feel free to explore the options given in `glm`, but remember to give a proper interpretation to your estimated coefficients. 
