...
ch_fit_ref <- emuFit(formula = ~ Group,
                     ...
                     constraint_fn = function(x) {x[ref_num]},
...
```{r}
radEmu_coef <- read.csv("radEmu_coef.csv")
...
Now we have estimates for all $758$ mOTUs, and robust score test results for the same mOTUs as earlier. The difference is that `radEmu` would take much longer to run these robust score tests in the presence of $758$ mOTUS.
...
We recommend using `fastEmu` over `radEmu` when running differential abundance analyses for very large datasets. It was motivated by the problem of estimating and testing log fold differences in abundance in genes from shotgun data. 
...
`ANCOM-BC2` is a differential abundance method built specifically for microbiome data, which accounts for sample-specific and taxon-specific detection effects in HTS data. 
...
When we look at `ancom_est` we can see six estimates are recorded as `NA`. This is because `ANCOM-BC2` cannot compute estimates and p-values for mOTUs that show separation, i.e. they only have non-zero counts in one of the covariate levels. This is one disadvantage of `ANCOM-BC2`. 

### DESeq2

`DESeq2` is another method that was built for RNA-seq data, but is commonly used for microbiome differential abundance analyses.  
...
`DESeq2` cannot run with default settings if all mOTUs have at least one zero. Let's check on this condition in our data. 
...
Finally, we can check out which mOTUs each method highlight as being the most significant

```{r}
pval_df %>%
  mutate(radEmu = rank(radEmu_pvals),
         ALDEx2 = rank(aldex_pvals),
         ANCOMBC2 = rank(ancom_pvals),
         DESeq2 = rank(deseq_pvals)) %>%
  filter(radEmu < 6 | ALDEx2 < 6 | ANCOMBC2 < 6 | DESeq2 < 6) %>%
  dplyr::select(-contains("pval")) %>%
  pivot_longer(cols = 2:5, names_to = "method", values_to = "rank") %>%
  mutate(rank = ifelse(rank > 5, NA, rank)) %>%
  ggplot(aes(y = mOTU, x = method, fill = rank)) + 
  geom_tile() + 
  labs(x = "Method", fill = "Rank")
```

From this plot we can see that there are $14$ mOTUs that have one of the top five smallest p-values for one or more method. Out of these, $10$ are only in the top five for one method, $2$ are in the top five for two methods, and $2$ are in the top five for three methods. If we were using these differential abundance methods for hypothesis generation, to find the mOTUs with the most evidence against the null hypotheses of no differential abundance, we would find some overlapping mOTUs across methods. 
