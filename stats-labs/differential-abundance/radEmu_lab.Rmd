...
qval_df %>%
  group_by(method) %>%
  summarise(sum(qval <= 0.10))
```

Here we can see that there are no mOTUs that are significant using qvalues and a false discovery rate of 0.10 using `radEmu` and `ALDEx2`, and there are $16$ and $25$ mOTUs that are significant using `ANCOM-BC2` and `DESeq2` respectively. This demonstrates a trade-off between these methods, `radEmu` and `ALDEx2` better control Type I error but often have lower power, while `ANCOM-BC2` and `DESeq2` fail to control Type I error in some settings but often have higher power. 

Finally, we will check how many of the significant mOTUs that were found by `ANCOM-BC2` and `DESeq2` are the same. 

```{r}
qval_df %>%
  filter(qval <= 0.10) %>%
  group_by(mOTU) %>%
  summarise(num_small_q = n()) %>%
  group_by(num_small_q) %>%
  summarise(n())
```

Here we can see that there are $23$ mOTUs that are only identified by one method and $9$ mOTUs that are identified by both methods. 
